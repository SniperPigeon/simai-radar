...
class SweepBurstTests(unittest.TestCase):
    ...
    def test_idle_speed_pressure_uses_each_hands_available_time(self):
        self.assertAlmostEqual(
            _idle_distance_pressure(4, 0.25, 6),
            4 * math.sqrt(8 / 3),
        )
        self.assertEqual(_idle_distance_pressure(3, 0.5, 6), 3)
        self.assertEqual(_idle_distance_pressure(0, 0, 6), 0)
