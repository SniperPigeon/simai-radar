...
180 BPM 等效八分的 1.5 次方加权。候选按强度取前五，名次继续按
`1/log2(x+1)` 衰减，并给 Top‑1 额外 1.3 倍；五条等强候选时 Top‑1 约占全部名次
权重的 40%。完整边界规则见
...
交互；普通 Tap/Hold 在低于 150 BPM 等效八分速度时按平方根温和下降，并使用双手动态
规划得到的平均最短外键位移作 0.7–1.0 修正。每个外部物件只归属一个最近的相关配置。完整公式见
