...
analysis 读取 ParseResult 的事件与时间字段，按显式配置调用独立分析器；每项返回原综合值及
可选 stats，不重新扫描 Simai 来计数。每个分析器使用独立实例和事件快照，失败
后继续其他维度。baseline、难度权重和映射算法不进入 parser；scoring 提供
ScoreTransformer 接口与按 feature 分发的 FeatureScoreTransformer；每个 feature 独立配置
映射器实例及其参数。

完整次序为 `analysis(data + stats) → 可选 fitted_constant → scoring → 雷达选轴`。
...
训练选列直接定义在 `regression/training.py` 的 `TRAINING_FEATURES`，展示选轴定义在
`exporters/visualizer.py` 的 `RADAR_FEATURES`；没有选列 CLI 或额外配置加载层。
改训练列表只影响后续训练，推理使用模型冻结的输入列，scorer 映射仍由评分层负责。
