...
普通 Tap/Hold 另使用只降不升的温和速度系数。令 `m_j` 为该配置内普通按钮出现的不同
时间点数（同拍双押只算一个时间点），`T_j` 为从 Slide 声明到最后一个普通按钮时间点的
秒数，等效八分 BPM 与速度系数为：

```text
B_eq_j = 30 * m_j / T_j
speed_j = sqrt(min(1, B_eq_j / 150))
```
...
排列为 `Q_(1), Q_(2), ...`，不足五个时补 0；`slide_tricky` 取前五强的算术平均：

```text
slide_tricky = (Q_(1) + Q_(2) + Q_(3) + Q_(4) + Q_(5)) / 5
```
