...
分析输出只提供综合值。子特征统计实现保存在 `constant_regression` 分支，见[消融报告](REGRESSION_ABLATION.md)。
...
CSV 使用 UTF-8 BOM，固定 metadata 列后追加综合量和可选预测值的 `<feature>_raw`，以及有 scorer
...
scorer 在模型预测之后执行，输入是综合量和可选 `fitted_constant`，都未经雷达映射。
